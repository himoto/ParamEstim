...
experiments[obs_idx("Phosphorylated_CREBw")] = Dict(
    "EGF" => [0,0.446,0.030,0.000,0.000],
    "HRG" => [0,1.000,0.668,0.460,0.340],
)
standard_error[obs_idx("Phosphorylated_CREBw")] = Dict(
...
function get_timepoint(observalbe::Int)
    if observalbe in [
        obs_idx("Phosphorylated_MEKc")
        obs_idx("Phosphorylated_ERKc")
        obs_idx("Phosphorylated_RSKw")
        obs_idx("Phosphorylated_cFos")
        ]
        return t2

    elseif observalbe == obs_idx("Phosphorylated_CREBw")
        return t3
    
    elseif observalbe == obs_idx("cfos_mRNA")
        return t4
        
    elseif observalbe in [
        obs_idx("cFos_Protein")
        obs_idx("dusp_mRNA")
        ]
        return t5
    end
